...
%estimating F and X
  F_hat=-(Qbar-G*inv(R)*G'/4)*del_phi'*w(:,k);
%F_hat=F;
zeta_hat(:,k+1)=zeta_hat(:,k)+dt*(F_hat+G*u(:,k)+K*(zeta(:,k)-zeta_hat(:,k)));
...
%Weight Updation
J_zeta=zeta_hat(:,k);
wdot=0.5*alpha*del_phi*G*inv(R)*G'*J_zeta-alpha*v*del_phi*(Qbar-G*inv(R)*G'/4)*(zeta(:,k)-zeta_hat(:,k));
w(:,k+1)=w(:,k)+dt*(wdot);
V1(k)=del_V'*Qbar*del_V+u(:,k)'*R*u(:,k);
