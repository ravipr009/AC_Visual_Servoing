...
cv::Mat theta_prev = cv::Mat(6,1, CV_64F, 0.0);
cv::Mat theta_doubledot = cv::Mat(6,1, CV_64F, 0.0);
cv::Mat theta = cv::Mat(6,1, CV_64F, 0.0);
cv::Mat del_theta = cv::Mat(6,1, CV_64F, 0.0);
cv::Mat rnn_w= cv::Mat(8,1, CV_64F, 0.0);
cv::Mat rnn_proj= cv::Mat(6,1, CV_64F, 0.0);
cv::Mat pixel_loc= cv::Mat(8,1, CV_64F, 0.0);
cv::Mat pixel_loc_D= cv::Mat(8,1, CV_64F, 0.0);
...
z_d=0.27;
for(int i=0;i<8;i++)
  pixel_loc_D.at<double>(i,0)=ud[i];
...
Zeta_PREV.at<double>(15,0)=Zeta.at<double>(15,0);
                   ...
                   rnn_w=rnn_w-(dt/epsi)*gam*(S-S_T)-(dt/epsi)*J_total*theta_dot;
                   std::cout<<"hi"<<std::endl;
                   ...
                   for(int i=0;i<6;i++)
                    { if(rnn_proj.at<double>(i,0)< (-3))
                       rnn_proj.at<double>(i,0)=-3;

                       if(rnn_proj.at<double>(i,0)>3)
                         rnn_proj.at<double>(i,0)=3;
                   }

//                   theta_dot=(dt/epsi)*(-theta_dot+rnn_proj)+theta_dot;
                   theta_dot=rnn_proj;
