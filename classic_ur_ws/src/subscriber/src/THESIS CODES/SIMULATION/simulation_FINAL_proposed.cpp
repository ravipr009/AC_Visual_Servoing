...
cv::Mat centre_rbf = cv::Mat(4,st/2  ,CV_64F,0.0);
...
int main(int argc, char** argv)
{
  ...
  myfile19.open("cost2_MYCODE.txt");
...
//(2) best out of 3: dt=0.01 (this was the one which was used last)
        double norme_z;
        sig.at<double>(0,0)=0.25; sig.at<double>(0,1)=0.5; sig.at<double>(0,2)=1; sig.at<double>(0,3)=1.25;
//        sig.at<double>(0,0)=1; sig.at<double>(0,1)=0.75; sig.at<double>(0,2)=0.5; sig.at<double>(0,3)=0.0000000001;

        for(int i=0; i<4; i++)
          for(int j=0;j<st/2;j++)
            centre_rbf.at<double>(i,j)=i*pow(10,4*i)*0.0000000000000001;
        std::cout<<"CENTRES"<<centre_rbf<<std::endl;
...
//            del_PHI.at<double>(i,j)= PHI.at<double>(0,i)*(0.5/(sig.at<double>(0,i)*sig.at<double>(0,i)))*2*Zeta.at<double>(j,0);
           ...
           for(int i=0;i<4;i++)
             for(int j=0;j<st/2;j++)
               del_PHI.at<double>(i,j)= PHI.at<double>(0,i)*(0.5/(sig.at<double>(0,i)*sig.at<double>(0,i)))*2*(Zeta.at<double>(j,0)-centre_rbf.at<double>(i,j));
